...
class Trainer:
    def __init__(self, config: Config):
        ...
        self.net_G = Generator(in_channels=config.input_dims).to(self.device)
        
        if config.LSGAN:
            self.net_D = Discriminator(use_sigmoid=False).to(self.device)
        else:
    ...
    def train_D(self, real_image: torch.Tensor, gauss_input: torch.Tensor):
        self.net_D.zero_grad()
        flip_label = random.random() < self.config.flip_label

        # Feed real images
        D_x = self.net_D(real_image)

        if not flip_label:
            real_label = torch.ones_like(D_x).to(self.device)
        else:
        ...
        # Feed fake images
        G_z = self.net_G(gauss_input)
        D_G_z = self.net_D(G_z)
    ...
    def train_G(self, real_image: torch.Tensor, gauss_input: torch.Tensor):
    ...
    def train_one_epoch(self):
        ...
        loss_D = 0.0
        loss_G = 0.0
        for idx, image in enumerate(self.train_loader):
            image = image.to(self.device)
            gauss = torch.randn(image.size(0), self.config.input_dims).to(self.device)
            
            # Train D
            loss_D += self.train_D(image, gauss)

            # train G
            loss_G += self.train_G(image, gauss)
    ...
    def train(self):
        try:
            for epoch in range(self.config.EPOCH):
                ...
                self.net_G.eval()
                outputs = self.net_G(self.fixed_noise)
                if epoch % 10 == 0:
            ...
            with open(os.path.join(self.config.exp_run_folder, self.config.training_log_savepath), mode = "a") as f:
                f.write(f",,,\n{best_epoch},,,{self.best_fid_value}\n")
            
            print(f"Model with FID minimum score is {self.best_fid_value} at epoch {best_epoch}")
